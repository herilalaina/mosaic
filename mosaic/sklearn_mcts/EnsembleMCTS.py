...
class EnsembleMTCS():
	...
	def train(self, X, y, D=None):
		...
		for name in self.list_model_name:
			if name in ["RandomForestClassifier", "XGBClassifier", "LogisticRegression", "SGDClassifier"]:
				models[name] = mcts_model(name, X, y, self.nb_play, self.nb_simulation, self.aggreg_score,
								self.init_ressource, self.init_nb_child, self.nb_step_add_ressource, self.nb_step_to_add_nb_child,
								self.ressource_to_add, self.number_child_to_add, self.cv)
			else:
				models[name] = mcts_model(name, X, y, self.nb_play, self.nb_simulation * self.acceleration, self.aggreg_score,
								self.init_ressource, self.init_nb_child, self.nb_step_add_ressource, self.nb_step_to_add_nb_child,
								self.ressource_to_add, self.number_child_to_add, self.cv)
		begin_bandit = 1
		...
		for i in range(self.nb_play):
			...
			if (not high_dimensional_data) and i >= begin_bandit and reward == 1 and score > val_scores and score > self.best_final_score:
	...
	def print_remaining_time(self, D):
		...
		print("\n\n---------------------------------------------------------------")
		print("             Time elapsed: {0}         Remaining time: {1}".format(spend, float(D.info['time_budget']) - spend))
		print("---------------------------------------------------------------\n\n")

	def update_reward(self, choosed_armed, reward, t):
		self.bandits_mean[choosed_armed] = self.bandits_mean[choosed_armed] + (reward - self.bandits_mean[choosed_armed]) / self.nb_visits[choosed_armed]
